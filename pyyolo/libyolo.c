...
void get_detection_info(image im, int num, float thresh, detection *boxes, int classes, char **names, list *output)
{
	int i;
	for(i = 0; i < num; ++i){
		detection d = boxes[i];
		int class = max_index(d.prob, classes);
		float prob = d.prob[class];
		if(prob > thresh){
			box b = d.bbox;
...
detection_info **yolo_detect(yolo_handle handle, image im, float thresh, float hier_thresh, int *num)
{
	...
	layer l = obj->net->layers[obj->net->n-1];
	detection* dets = get_network_boxes(obj->net, im.w, im.h, thresh, hier_thresh, 0, 1, num);
	// get_region_boxes(l, im.w, im.h, obj->net->w, obj->net->h, thresh, obj->probs, obj->boxes, NULL, 0, 0, hier_thresh, 1);
	if (obj->nms) do_nms_obj(dets, *num, l.classes, obj->nms);
	list *output = make_list();
	get_detection_info(im, *num, thresh, dets, l.classes, obj->names, output);
...
detection_info **yolo_test(yolo_handle handle, char *filename, float thresh, float hier_thresh, int *num, float **feature_map, int *map_size)
{
	...
	*map_size = obj->net->outputs;
	...
	list *output = make_list();
	get_detection_info(im, l.w*l.h*l.n, thresh, dets, l.classes, obj->names, output);
	detection_info **info = (detection_info **)list_to_array(output);
	// *num = nboxes
	*num = output->size;
